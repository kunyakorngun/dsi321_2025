...
# โหลดไฟล์ CSV
df = pd.read_csv("scrap_data.csv", header=0, encoding='utf-8-sig')
...
for _, row in df.iterrows():
    ...
    keyword = row["keyword"]
    published = row["published"] if "published" in df.columns else None  # ดึง published ถ้ามี
    ...
    filtered_rows.append({
        ...
        "keyword": keyword,
        "published": published  # ✨ เพิ่มคอลัมน์นี้
    })

# สร้างไฟล์ใหม่
filtered_df = pd.DataFrame(filtered_rows)
filtered_df.to_csv("filtered_by_topic.csv", index=False, encoding="utf-8") 
